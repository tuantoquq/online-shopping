...
instance.interceptors.response.use(
  ...
  async (err) => {
    ...
    if (originalConfig.url !== (BASE_API_URL + "/shopper/login") && err.response) {
      // Access Token was expired
      if (err.response.status === 401 && !originalConfig._retry) {
        originalConfig._retry = true;
        try {
          const rs = await axios.post(BASE_API_URL + "/shopper/refresh-token", {
          ...
          if (rs.data.status === 0) {
            ...
            TokenService.updateLocalAccessToken(ROLE.shopper, `Bearer ${accessToken}`);
