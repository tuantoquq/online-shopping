...
instance.interceptors.request.use(
  (config) => {
    const token = TokenService.getLocalAccessToken(ROLE.admin);
    if (token) {
...
instance.interceptors.response.use(
  ...
  async (err) => {
    ...
    if (originalConfig.url !== BASE_API_URL + '/admin/login' && err.response) {
      // Access Token was expired
      if (err.response.status === 401 && !originalConfig._retry) {
        originalConfig._retry = true;
        try {
          const rs = await axios.post(BASE_API_URL + '/admin/refresh-token', {
            headers: {
              'Content-Type': 'application/json',
          ...
          if (rs.data.status === 0) {
            ...
            TokenService.updateLocalRefreshToken(ROLE.admin, refreshToken);
          } else {
