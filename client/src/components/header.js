...
function Header() {
  ...
  useEffect(() => {
    const handlleScroll = () => {
      ...
      const x1 = document.querySelector(`.${styles.narBar}`)?.clientHeight;
      const x2 = document.querySelector(`.${styles.toolBar}`)?.clientHeight;
  ...
  return (
    <div className={styles.Header}>
      ...
      {VisibleMenuAll && (
        <div className={styles.rowMenu} style={{ top: VisibleMenuTop }}>
          <div className={`${styles.rowMenu_1} ${styles.textColor}`}>
            <ul>
              <h2>Thời sự</h2>
              {topicThoiSu.map((topic) => (
                <li>
                  <a href={`/${topic[1]}`}>{topic[0]}</a>
                </li>
              ))}
            </ul>
            <ul>
              <h2>Góc nhìn</h2>
              {toppicGocNhin.map((topic) => (
                <li>
                  <a href={`/${topic[1]}`}>{topic[0]}</a>
                </li>
              ))}
